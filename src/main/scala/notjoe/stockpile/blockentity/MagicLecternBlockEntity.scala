...
import notjoe.stockpile.blockentity.MagicLecternBlockEntity._
...
class MagicLecternBlockEntity() extends BlockEntity(Type) {
  ...
  def bookshelfRingExists(centerPos: BlockPos, radius: Int): Boolean = {
    ...
    positions.count { case (x, z) =>
      world.getBlockState(centerPos.add(x, 0, z)).getBlock == Blocks.BOOKSHELF
    } >= RequiredBookshelvesInRadius
  }
}
